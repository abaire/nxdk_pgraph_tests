...
Prefer adding new tests that align thematically with existing suites to those suites. You may use the
`create_test_suite.py` script in the `utils` subdirectory to generate skeletal test suites.

### Documentation

Autogenerated documentation may be accessed at https://abaire.github.io/nxdk_pgraph_tests

Some special Doxygen comments are used
by [the results site generator project](https://github.com/abaire/xemu-nxdk_pgraph_tests_results) to describe test
results on the website:

1. A detailed description attached to the `TestSuite` subclass (see `two_d_line_tests.h`)
2. Ideally a list of `@tc` custom commands in the documentation for the constructor method, explaining what each test is
   doing and/or is expected to display (see `two_d_line_tests.cpp`).
...
See `two_d_line_tests.h` for an example.
