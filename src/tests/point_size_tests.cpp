...
static const TestConfig testConfigs[]{
    {"PointSmoothOn_1_FF", true, 1, false},   {"PointSmoothOff_1_FF", false, 1, false},
    {"PointSmoothOn_8_FF", true, 8, false},   {"PointSmoothOff_8_FF", false, 8, false},
 ...
 *   by 16 per point starting at the upper left point. Demonstrates that point size only affects the fixed function
 * pipeline.
 *
...
static void RenderPoints(RenderPointCB&& on_render_point) {
  auto z = 0.f;
  float red = 0.1f;
  for (auto y = 100; y < 480; y += 20) {
    float blue = 0.1f;
    for (auto x = 20; x < 640; x += 40) {
