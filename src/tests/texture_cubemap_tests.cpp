...
static constexpr const char kDotSTRTest[] = "DotSTRCubemap";
...
TextureCubemapTests::TextureCubemapTests(TestHost &host, std::string output_dir)
  ...
  tests_[kDotSTRTest] = [this]() { TestDotSTRCubemap(); };
}
...
void TextureCubemapTests::TestDotSTRCubemap() {
  ...
  host_.FinishDraw(allow_saving_, output_dir_, kDotSTRTest);
}
