...
void GenerateRGBRadialATestPattern(void *target, uint32_t width, uint32_t height) {
  ...
  auto lr_pixels = ll_pixels + half_width;
  ...
  GenerateRGBTestPattern(ll_pixels, half_width, half_height, 0x00, width);
  GenerateRGBTestPattern(lr_pixels, half_width, half_height, 0x00, width);
  ...
  for (uint32_t y = 0; y < height; ++y) {
    ...
    for (uint32_t x = 0; x < width; ++x, ++pixels) {
      ...
      auto alpha = 255.0f * (1.f - distance / max_distance);

      *pixels = (*pixels & 0x00FFFFFF) + (static_cast<uint32_t>(alpha) << 24);
    }
  }
}
